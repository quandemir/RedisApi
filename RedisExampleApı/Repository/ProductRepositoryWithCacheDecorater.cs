...
namespace RedisExampleApı.Repository
{
    public class ProductRepositoryWithCacheDecorater : IProductRepository
    {
        private const string productKey= "productCaches"; //redis keÿismi 
        ...
        public async Task<Product> CreateAsync(Product product)
        {
            ...
            if(await _cacheRepository.KeyDeleteAsync(productKey))
            {
        ...
        public async Task<List<Product>> GetAsync()
        {
            ...
            var cachProducts = await _cacheRepository.HashGetAllAsync(productKey);
            var products =new List<Product>();
            foreach (var item in cachProducts.ToList())
            {
